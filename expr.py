...
from token import Token, PseudoRuntimeError, PseudoTypeError, PseudoNameError
...
class UnaryExpression(Expression):
    ...
    def eval(self, ctx):
        if self.operation == '-':
            ...
            if arg.type != 'number':
                raise PseudoTypeError(self.context, '-({}) not supported'.format(arg.type))

            return Token('number', -arg.value)
        
        else:
            raise PseudoRuntimeError(self.context, "Unsupported unary expression")
...
class BinaryExpression(Expression):
    ...
    def eval(self, ctx):
        ...
        if self.operation == '+':
            ...
            target = self.argument1
            value = Expression._get_arg(ctx, self.argument2)
            
            if not isinstance(target, VariableReference):
                raise PseudoTypeError(self.context, "Assignment target not a variable reference")
            
            target.set(ctx, value)
            return value
            
        elif self.operation in ('=', '==', 'eq', 'equals'):
            res = self._do_operation(ctx, 'number', lambda a,b: int(a == b))
            
        elif self.operation in ('<', 'lt'):
            res = self._do_operation(ctx, 'number', lambda a,b: int(a < b))
            
        elif self.operation in ('>', 'gt'):
            res = self._do_operation(ctx, 'number', lambda a,b: int(a > b))
            
        elif self.operation in ('<=', 'le'):
            res = self._do_operation(ctx, 'number', lambda a,b: int(a <= b))
            
        elif self.operation in ('>=', 'ge'):
            res = self._do_operation(ctx, 'number', lambda a,b: int(a >= b))
